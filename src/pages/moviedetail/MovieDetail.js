...
export function MovieDetail() {
    ...
    useEffect(() => {
        fetch(`https://www.omdbapi.com/?i=${id}&apikey=fcd454e6`)
            .then(res => res.json())
            .then(apiData => {
                    ...
                    <div className='movie-detail-btns'>
                        {list.find(item => item.imdbID === movieDeatil.imdbID) ?
                            <button
                                ...
                                className='bg-nored'>- REMOVE FROM LIST</button> :
                            <button
                                onClick={() => dispatch(addToList(movieDeatil))}
                                className='bg-red'>+ ADD TO LIST</button>}
                        <a target={"_blank"} rel="noreferrer"
                            href={`https://www.imdb.com/title/${movieDeatil.imdbID}/`}>
                            <button className='bg-3a70cf '>More info in IMDB</button>
                        </a>
